/*
 * WSOLED_XIAO — nodo camera del sistema camper (Seeed XIAO ESP32-S3 Sense)
 * ---------------------------------------------------------------------------
 ...
 * l'aggiornamento OTA come sul template C3 (un nodo montato in un gavone non
 * si raggiunge col cavo).
 *
 ...
 *   hub_link.*  ESP-NOW verso l'hub, sopra la libreria WSOLED_Link
 *
 * DIPENDE DA libraries/ (WSOLED_Link): se sposti questa cartella fuori dal
 * repo, portati dietro anche libraries/WSOLED_Link (o una junction). A
 * differenza di WSOLED_C3/, questo template NON e' self-contained: usa il
 * protocollo condiviso dell'hub apposta, per non duplicarlo.
 *
...
void setup() {
  ...
  Serial.println();
  Serial.printf("=== WSOLED_XIAO — nodo camera \"%s\" %s ===\n", NODE_NAME, FW_VERSION);
