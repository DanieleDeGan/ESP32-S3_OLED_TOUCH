...
> **Unverified against this project:** `README.md` and `WSOLED.ino` in this repo document the **V1 SPI** wiring (CLK=47 shared with the display, MOSI=42, MISO=8, CS=9) as the one to follow, pointing to Waveshare's `SD_Test` demo. No SD code has actually been written/tested here yet, so this hasn't been confirmed against real hardware — check the physical board revision (or just try SDMMC first, per above) before writing SD code, since the two wirings are not interchangeable.
